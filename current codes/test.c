...
#define CLK_PIN   BIT0      // P2.0
#define DATA_PIN1  BIT1      // P2.1
#define DATA_PIN2  BIT2      // P2.2

#define LED_STATUS   BIT2      // LED on P3.2
// #define LED_PIN2   BIT3      // LED on P3.3
// #define LED_PIN3   BIT4      // LED on P3.4
// #define LED_PIN4   BIT5      // LED on P3.5
...
int main(void)
{
    ...
    P2DIR &= ~(CLK_PIN | DATA_PIN1);
    P2DIR |= DATA_PIN2; //output at first
    P2OUT |= DATA_PIN2; //default high
    ...
    while (1) {
        ...
        while (waiting){
            clk_now = (P2IN & CLK_PIN) ? 1 : 0;
            // detect rising edge: last = 0, now = 1
            if (clk_now && !last_clk) {
                // sample DATA on rising edge
                unsigned char bit1 = (P2IN & DATA_PIN1) ? 1 : 0;
                if (bit1 == 1){
                    waiting = 0;
                    P2OUT &= ~DATA_PIN2; //off
                    P2DIR |= DATA_PIN2;
        ...
        if (clk_now && !last_clk) {
            // sample DATA on rising edge
            unsigned char bit1 = (P2IN & DATA_PIN1) ? 1 : 0;
            unsigned char bit2 = (P2IN & DATA_PIN2) ? 1 : 0;
